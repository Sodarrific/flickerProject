...
public class animationsControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            actionQuery = false;
            GetComponent<Animator>().SetBool("swing1", false);
            GetComponent<Animator>().SetBool("swing2", false);
        }
        ...
        if (Input.GetKeyDown("f") && monster == true && actionQuery == false)
        {
            print("f key pressed");
            if (swingType == true)
            {
                print("swing 1");
                GetComponent<Animator>().SetBool("swing1", true);
                swingType = false;
                actionTimer = -2.0;
                actionQuery = true;
            }
            else if (swingType == false)
            {
                print("swing 2");
                GetComponent<Animator>().SetBool("swing2", true);
                swingType = true;
                actionTimer = -2.0;
                actionQuery = true;
            }
